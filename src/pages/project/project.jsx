...
const cards = timeRanges.map((timeRange) => {
    return <CardTimeRange 
        key={timeRange._id}
        id={timeRange._id}
        title={timeRange.title}
        description={`${new Date(timeRange.startDate).toLocaleDateString()} ↔  ${new Date(timeRange.endDate).toLocaleDateString()}`}
    />;
})
...
timeRanges.forEach((timeRange) => {
    numberOfNodes.push({window: timeRange.title, frequency: timeRange.network.networkMetrics.numberOfNodes});
    numberOfEdges.push({window: timeRange.title, frequency: timeRange.network.networkMetrics.numberOfEdges});
    density.push({window: timeRange.title, frequency: timeRange.network.networkMetrics.density});
    ...
    reciprocity.push({window: timeRange.title, frequency: timeRange.network.networkMetrics.reciprocity});
    degreeCentrality[0].data.push({x: timeRange.title, y: timeRange.network.networkMetrics.degreeCentrality});
    radius[0].data.push({x: timeRange.title, y: timeRange.network.networkMetrics.radius});
    diameter[0].data.push({x: timeRange.title, y: timeRange.network.networkMetrics.diameter});
});
                ...
                <div className='network-evolution'>
                    ...
                    <div className='charts-list'>
                        <div className='chart-container'>
                            ...
                            <BarChart width={100*numberOfNodes.length} height={400} data={numberOfNodes}/>
                        </div>
                        <div className='chart-container'>
                            <div className='small-title-project'>Number of Edges Evolution</div>
                            <BarChart width={100*numberOfEdges.length} height={400} data={numberOfEdges}/>
                        </div>
                        <div className='chart-container'>
                        ...
                        <div className='chart-container'>
                            ...
                            <BarChart width={100*reciprocity.length} height={400} data={reciprocity}/>
                        </div>
                ...
                <div className='source-network'>
                    ...
                    <div className='charts-list'>
                        <div className='chart-container'>
                            ...
                            <PieChart width={300} height={300} dataObject={{
                                                                        retweets: project.sourceNetwork.retweetNetworkMetrics.numberOfEdges,
                                                                        quotes: project.sourceNetwork.quoteNetworkMetrics.numberOfEdges,
                                                                    }} />
                        </div>
                        <div className='chart-container'>
